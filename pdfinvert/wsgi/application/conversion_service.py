    ...
    def convert(self, filepath: str, dpi: int) -> str:
        ...
        for i in range(0, page_count):
            subprocess.check_call(
                f"convert -density 600 -flatten {filepath}[{i}] {flattened_file.strip('.pdf')}-{i:04d}.pdf ".split(
                    " ")[:-1])
            self.temporary_file_store.add(f"{flattened_file.strip('.pdf')}-{i:04d}.pdf")
            subprocess.check_call(
                f"convert -density 600 -negate {flattened_file.strip('.pdf')}-{i:04d}.pdf {flattened_file.strip('.pdf')}-{i:04d}.inverted.pdf ".split(
                    " ")[:-1])
            self.temporary_file_store.add(f"{flattened_file.strip('.pdf')}-{i:04d}.inverted.pdf")

        subprocess.check_call(
            f"convert -density 600 {flattened_file.strip('.pdf')}-%04d.inverted.pdf[0-{page_count - 1}] {final_file} ".split(
                " ")[:-1])
